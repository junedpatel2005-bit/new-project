          ...
          visible.map((job) => (
            <article key={job.id} className="rounded-xl border bg-card p-5">
              <div className="flex flex-wrap justify-between gap-3">
                <div>
                  <span
                    className={`rounded-full px-2.5 py-1 text-xs font-semibold ${job.status === "OPEN" ? "bg-green-100 text-green-800" : job.status === "DRAFT" ? "bg-amber-100 text-amber-800" : "bg-muted"}`}
                  >
                ...
                <div className="flex gap-2">
                  {job.status === "DRAFT" && (
                    <>
                      ...
                      <Button variant="outline" onClick={() => void remove(job.id)}>
                        Delete draft
                      </Button>
                    </>
                  )}
